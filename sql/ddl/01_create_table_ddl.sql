...
CREATE TABLE IF NOT EXISTS `sena_db`.`debit_balances` (
  ...
  `stock_code` INT NOT NULL COMMENT '銘柄コード',
  `margin_selling_balance` INT NOT NULL COMMENT '信用売残',
  `margin_debt_balance` INT NOT NULL COMMENT '信用買残',
  `ratio_margin_balance` DECIMAL(15,5) NOT NULL COMMENT '信用倍率（整数部：10桁、小数部：5桁）',
  `created` DATETIME NOT NULL COMMENT '作成日時',
  `modified` DATETIME NOT NULL COMMENT '更新日時',
  `delete_flg` TINYINT(1) NOT NULL DEFAULT 0 COMMENT '削除フラグ（1：削除、0：未削除）',
  PRIMARY KEY (`id`))
...
CREATE INDEX `i_debit_balances1` ON `sena_db`.`debit_balances` (`delete_flg` ASC, `stock_code` ASC);
...
CREATE TABLE IF NOT EXISTS `sena_db`.`indicators` (
  ...
  `date` DATE NOT NULL COMMENT '取得日',
  `stock_code` INT NOT NULL COMMENT '銘柄コード',
  `dividend_yield` DECIMAL(15,5) NULL COMMENT '配当利回り（整数部：10桁、小数部：5桁）',
  `price_earnings_ratio` DECIMAL(15,5) NULL COMMENT '株価収益率（PER、整数部：10桁、小数部：5桁）',
  `price_book_value_ratio` DECIMAL(15,5) NULL COMMENT '株価純資産倍率（PBR、整数部：10桁、小数部：5桁）',
  `earnings_per_share` DECIMAL(15,5) NULL COMMENT '1株利益（EPS、整数部：10桁、小数部：5桁）',
  `book_value_per_share` DECIMAL(15,5) NULL COMMENT '1株当たり純資産（BPS、整数部：10桁、小数部：5桁）',
  `return_on_equity` DECIMAL(25,20) NULL COMMENT '株主資本利益率（ROE、整数部：5桁、小数部：20桁）',
  `capital_ratio` DECIMAL(15,5) NULL COMMENT '自己資本比率（整数部：10桁、小数部：5桁）',
  `created` DATETIME NOT NULL COMMENT '作成日時',
  `modified` DATETIME NOT NULL COMMENT '更新日時',
  `delete_flg` TINYINT(1) NOT NULL DEFAULT 0 COMMENT '削除フラグ（1：削除、0：未削除）',
  PRIMARY KEY (`id`))
...
CREATE TABLE IF NOT EXISTS `sena_db`.`finances` (
  ...
  `sales_rate` DECIMAL(25,20) NULL COMMENT '売上高（前年比、整数部：5桁、小数部：20桁）',
  `operating_profit_rate` DECIMAL(25,20) NULL COMMENT '営業益（前年比、整数部：5桁、小数部：20桁）',
  `net_profit_rate` DECIMAL(25,20) NULL COMMENT '純利益（前年比、整数部：5桁、小数部：20桁）',
  `created` DATETIME NOT NULL COMMENT '作成日時',
  `modified` DATETIME NOT NULL COMMENT '更新日時',
  `delete_flg` TINYINT(1) NOT NULL DEFAULT 0 COMMENT '削除フラグ（1：削除、0：未削除）',
  PRIMARY KEY (`id`))
